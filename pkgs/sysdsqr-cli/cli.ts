...
type CommandInput<TFlags = Record<string, never>, TArgs extends Array<string> = Array<string>> = {
  flags: TFlags;
  args: TArgs;
  ctx: CliContext;
};

type CommandSpec<TFlags = Record<string, never>, TArgs extends Array<string> = Array<string>> = {
  description?: string;
  commands?: CommandMap;
...
const groupCommands = (argv: Array<string>, commands: CommandMap): Array<ParsedCommand> => {
  const { result, current } = argv.reduce<{ result: Array<ParsedCommand>; current: ParsedCommand | null }>(
...
const command = <TFlags = Record<string, never>, TArgs extends Array<string> = Array<string>>(
