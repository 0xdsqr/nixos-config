_: {
  flake.commonModules.theme =
    ...
    let
      ...
      selectedTheme =
        themeCatalog.${config.dsqr.theme.id}
          or (throw "Unknown theme '${config.dsqr.theme.id}'. Available themes: ${builtins.concatStringsSep ", " (builtins.attrNames themeCatalog)}");
    ...
    in
    {
      options.dsqr.theme.id = mkOption {
        type = str;
        default = "dsqr";
