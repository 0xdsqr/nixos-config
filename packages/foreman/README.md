...
`foreman` is a small Nushell-first CLI for working on this `nixos-config` repository.
...
- `foreman version` prints the packaged CLI version.
- `foreman secrets sync` runs `agenix -i ~/.ssh/dsqr_homelab_ed25519 -r`.
